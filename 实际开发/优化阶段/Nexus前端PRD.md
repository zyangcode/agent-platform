...
| 角色 | 描述 | 使用场景 |
|------|------|---------|
| **User（普通用户）** | 业务人员、产品经理 | 选 Agent 对话、上传个人 Skill、查看自己的用量和历史 |
| **Developer（开发者）** | 后端/全栈工程师 | 创建 Agent Profile、配置 Prompt + Skill + MCP、上传 Jar Skill、通过 API Key 接入代码 |
| **Admin（管理员）** | IT 管理员 | 管理模型供应商、全局 Skill 上架/下架、安全策略、Token 配额、用户管理、告警配置 |

### 1.3 两套界面

| 端 | 用户 | 入口 |
|------|------|------|
| **User 端** | User + Developer | 登录后进入对话工作台，左侧导航切功能 |
| **Admin 端** | Admin | 独立管理后台，全局配置和监控 |
