...
MVP 如果不启用 pgvector，可以先不建 `embedding` 字段，或保留为 NULL 注释字段。
...
| `token_usage_logs` | `(tenant_id, application_id, created_at)`、`(tenant_id, user_id, created_at)`、`trace_id` |
| `trace_roots` | `trace_id unique`、`(tenant_id, application_id, started_at)` |
